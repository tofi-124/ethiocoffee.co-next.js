...
export default function EthiopianCoffeeSampleApprovalGuide({ title, large_image_url }: InsightContentProps) {
  return (
    <>
      ...
      <p className='my-4 text-lg leading-relaxed'>
        That is why this guide focuses on operating procedure rather than on cupping basics alone. If you need a detailed sensory method, use our <Link href='/insights/how-to-cup-evaluate-ethiopian-coffee-samples' className='text-amber-700 hover:underline font-bold'>Ethiopian coffee cupping guide</Link>. Before the panel makes a release decision, the <Link href='/insights/coffee-cupping-calibration-team-protocol' className='text-amber-700 hover:underline font-bold'>coffee cupping calibration protocol</Link> shows how to check score drift and hidden-duplicate repeatability. If a landed lot no longer matches the approved reference, use our <Link href='/insights/green-coffee-quality-claims-guide' className='text-amber-700 hover:underline font-bold'>green coffee quality claims guide</Link>. Here, the goal is simpler: build a repeatable approval system that prevents weak contracts and avoidable arrival disputes.
      </p>
      ...
      <p className='my-4'>
        Good buyers separate selection, shipment authorization, and landed verification. Each stage has a different question, a different response time, and a different standard of evidence. Each stage also assumes you can measure, roast, and cup under repeatable conditions. Use the <Link href='/insights/green-coffee-sample-roasting-protocol' className='text-amber-700 hover:underline font-semibold'>green coffee sample roasting protocol</Link> to keep preparation comparable across all three checkpoints; the <Link href='/insights/green-coffee-quality-control-lab-setup-guide' className='text-amber-700 hover:underline'>green coffee quality control lab setup guide</Link> shows how to build that capability at three budget levels.
      </p>
